...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.assignStudent.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.assign-students.update", [$assignStudent->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('name') ? 'is-invalid' : '' }}" name="name_id" id="name_id" required>
                    @foreach($names as $id => $entry)
                        <option value="{{ $id }}" {{ (old('name_id') ? old('name_id') : $assignStudent->name->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('master') ? 'is-invalid' : '' }}" name="master_id" id="master_id" required>
                    @foreach($masters as $id => $entry)
                        <option value="{{ $id }}" {{ (old('master_id') ? old('master_id') : $assignStudent->master->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('route') ? 'is-invalid' : '' }}" name="route_id" id="route_id" required>
                    @foreach($routes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('route_id') ? old('route_id') : $assignStudent->route->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('route'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('vehicle_name') ? 'is-invalid' : '' }}" name="vehicle_name_id" id="vehicle_name_id" required>
                    @foreach($vehicle_names as $id => $entry)
                        <option value="{{ $id }}" {{ (old('vehicle_name_id') ? old('vehicle_name_id') : $assignStudent->vehicle_name->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('stop_name') ? 'is-invalid' : '' }}" name="stop_name_id" id="stop_name_id" required>
                    @foreach($stop_names as $id => $entry)
                        <option value="{{ $id }}" {{ (old('stop_name_id') ? old('stop_name_id') : $assignStudent->stop_name->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fromdate') ? 'is-invalid' : '' }}" type="text" name="fromdate" id="fromdate" value="{{ old('fromdate', $assignStudent->fromdate) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('todate') ? 'is-invalid' : '' }}" type="text" name="todate" id="todate" value="{{ old('todate', $assignStudent->todate) }}" required>
