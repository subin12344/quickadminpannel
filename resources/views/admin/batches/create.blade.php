    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.batches.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_leaving') ? 'is-invalid' : '' }}" type="text" name="date_leaving" id="date_leaving" value="{{ old('date_leaving') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('syllabus_year') ? 'is-invalid' : '' }}" name="syllabus_year_id" id="syllabus_year_id" required>
                    @foreach($syllabus_years as $id => $entry)
                        <option value="{{ $id }}" {{ old('syllabus_year_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
