...
@can('toolssyllabus_year_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.toolssyllabus-years.create') }}">
                        ...
                        <tr data-entry-id="{{ $toolssyllabusYear->id }}">
                            ...
                            <td>
                                @can('toolssyllabus_year_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.toolssyllabus-years.show', $toolssyllabusYear->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('toolssyllabus_year_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.toolssyllabus-years.edit', $toolssyllabusYear->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('toolssyllabus_year_delete')
                                    <form action="{{ route('admin.toolssyllabus-years.destroy', $toolssyllabusYear->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
