...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.transportStop.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.transport-stops.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('route_name') ? 'is-invalid' : '' }}" name="route_name_id" id="route_name_id" required>
                    @foreach($route_names as $id => $entry)
                        <option value="{{ $id }}" {{ old('route_name_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
